...
- `RUNPOD_AUTHED_GRAPHQL_URL`: override the GraphQL host for authenticated, no-REST-equivalent operations. Sends the caller's API key — point it only at a trusted host. Used by `deploy-hub-repo`, `set-endpoint-gpus`, and `get-endpoint` with `includeGpuIds`. `update-endpoint` deliberately does not perform a GraphQL pre-read: no read can make its later REST PATCH atomic. (Its only pre-read is a same-host REST GET that exempts CPU endpoints from the GPU-replacement requirement — compute type is fixed at creation, so that read cannot go stale.) Cross-API reads trust only the production defaults automatically: a one-sided override is a known mismatch, while two custom URLs are merely unverified and require `allowUnverifiedEnvironmentPair:true` before `get-endpoint` merges them. Exclusion validation in `set-endpoint-gpus` fails closed outside a verified pair unless `allowUnvalidatedExclusions:true` explicitly accepts the risk.
