...
- `RUNPOD_AUTHED_GRAPHQL_URL`: override the GraphQL host for authenticated, no-REST-equivalent operations. Sends the caller's API key — point it only at a trusted host. Used by `deploy-hub-repo`, `set-endpoint-gpus`, `get-endpoint` with `includeGpuIds`, and **every v2 `update-endpoint`** (which reads `gpuIds` to protect GPU SKU exclusions the v2 REST model cannot represent).
