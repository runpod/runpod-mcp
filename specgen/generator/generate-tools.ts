...
const defs: Record<string, unknown> = inlineRefs(spec.components?.schemas ?? {});
...
function reachableDefs(fragment: unknown): Record<string, unknown> {
  ...
  const queue = [...JSON.stringify(fragment ?? {}).matchAll(DEF_REF)].map((m) => m[1]);
  while (queue.length) {
...
for (const [path, pathItem] of Object.entries<Record<string, any>>(spec.paths)) {
  for (const method of HTTP_METHODS) {
    ...
    for (const raw of rawParams) {
      const param = raw.$ref
        ? spec.components.parameters[raw.$ref.replace("#/components/parameters/", "")]
        : raw;
    ...
    const description =
      ...
      [op.summary, op.description].filter(Boolean).join(". ").replaceAll(/\s+/g, " ").trim();
