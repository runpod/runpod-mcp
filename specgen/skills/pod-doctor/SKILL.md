...
- **Stop vs terminate.** State the consequence before acting, every time: **stopping clears the container disk**, releases GPU/CPU compute, and moves the Pod to `EXITED`. The persistent volume disk (`mounts.persistent`) survives stop and continues billing for storage; a network volume (`mounts.network`) persists independently. Before honoring "stop but keep my data", inspect mounts and verify the needed files are on persistent storage. Copy anything needed from the container disk and verify the copy before stopping; if you cannot do that with the available tools, explain the blocker. Starting resumes the Pod but cannot recover its erased container files. **Terminate** permanently deletes the Pod and its local persistent volume disk; a network volume is only detached. Terminate only on an explicit "delete / terminate / throw it away", and say what data is lost.
