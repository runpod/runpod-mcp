...
- **Create then pause for later.** When the user wants the Pod paused ("I'll continue tomorrow, keep my data"), first inspect its storage mounts and verify that files they need are on a persistent volume disk or network volume. Copy needed files off the container disk and verify the copy before stopping; if the available tools cannot do that, explain the blocker before proceeding. **Stopping clears the container disk.** A persistent volume disk remains attached to the stopped Pod and continues billing for storage; a network volume persists independently. Stop only after the data is safe, then read back the stopped state and explain that compute billing ends but persistent storage still bills. Never substitute termination: it also deletes the Pod's local persistent volume disk. Resume with start; capacity availability can affect resumption.
