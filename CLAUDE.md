...
The server communicates with two separate Runpod API backends. The REST API at `https://rest.runpod.io/v1` handles all authenticated CRUD operations for Pods, endpoints, templates, network volumes, and container registry auths. It requires a `RUNPOD_API_KEY` environment variable. The GraphQL API at `https://api.runpod.io/graphql` is reached two ways. The public, unauthenticated path serves read-only discovery queries — GPU types, data centers, the Hub catalog, Public Endpoints. The authenticated path (`graphqlAuthed`, API key as a Bearer token) serves the operations that have no REST equivalent: `deploy-hub-repo` and `set-endpoint-gpus` (both via the `saveEndpoint` mutation), plus the read-only `get-endpoint includeGpuIds` enrichment. The v2 REST model cannot represent GPU SKU exclusions, so `update-endpoint` does not attempt a cross-API read/write sequence: every v2 update of a GPU endpoint requires non-empty `gpuPoolIds`, a positive integer `gpuCount`, and `replaceGpuSelection: true` until the REST facade preserves `gpuIds` when `gpu` is omitted. CPU serverless endpoints are exempted by a same-host REST read — they have no `gpuIds`, and compute type is fixed at creation, so that read has none of the staleness an exclusion pre-read would have. (The current facade still rejects CPU PATCHes server-side; the tool wraps that 400 with the CPU context and the `RUNPOD_REST_VERSION=v1` escape rather than refusing client-side, so no MCP change is needed when the facade gains CPU update support.) The public and authenticated GraphQL paths resolve their hosts from separate env vars on purpose — see `RUNPOD_PUBLIC_GRAPHQL_URL` and `RUNPOD_AUTHED_GRAPHQL_URL` below.
...
- `RUNPOD_AUTHED_GRAPHQL_URL`: override the GraphQL host for **authenticated** operations with no REST equivalent — `deploy-hub-repo`, `set-endpoint-gpus`, and `get-endpoint` with `includeGpuIds` (default `https://api.runpod.io/graphql`). These send the caller's API key as a Bearer token, so only point this at a host you trust with it; on the hosted server that key is a per-user OAuth-minted one. The production REST/GraphQL defaults are the only verified environment pair. One custom host is a known mismatch; two custom hosts are unverified rather than assumed paired. Cross-API reads or exclusion checks must fail closed unless their tool exposes and receives the corresponding explicit unsafe acknowledgement.
