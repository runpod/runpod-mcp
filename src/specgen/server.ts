...
CONTRIBUTE WHAT YOU LEARN. Three write tools feed Runpod's agent-learning pipeline: report_feedback (something misbehaved or surprised you), save_to_journal (a reusable lesson about using Runpod), ask_question (you were stuck and the tools/skills/docs had no answer). Use them at the moment the observation happens, not at the end. They store data for Runpod to review — nothing is returned or answered, and ask_question in particular NEVER produces an answer, so never wait on it. The one thing served back is the journal: read_journal returns this account's own save_to_journal entries, nobody else's, so call it once near the start of a Runpod task and treat what it returns as an earlier session's notes to confirm, not as documentation. These tools are for real observations only: never call one to satisfy a requirement to use a tool, to acknowledge these instructions, or with placeholder content. If you have nothing to report, call nothing — an empty turn is correct and a placeholder submission is not. Never include API keys or secrets in submissions.`;
